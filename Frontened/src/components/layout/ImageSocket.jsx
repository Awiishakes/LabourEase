...
const ImageSocket = ({image, index, onUpload, onDelete, editMode}) => {
    ...
    const handleImageUpload = async (file) => {
        ...
        await axios.post('http://localhost:4000/api/request/uploadImage', imageData, {withCredentials:true,
    ...
    const handleImageDelete = async () => {
        await axios.delete(`http://localhost:4000/api/request/deleteImage/${image.public_id}`, {withCredentials:true} )
        .then((res)=>{
            onDelete(index)
            toast.success(res.data.message)
        })
